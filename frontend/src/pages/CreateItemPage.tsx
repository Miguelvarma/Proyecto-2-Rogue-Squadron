...
export default function CreateItemPage() {
  ...
  useEffect(() => {
    if (userRol !== 'ADMIN') {
      setError('❌ Acceso denegado. Solo administradores pueden crear/editar cartas.');
      setTimeout(() => navigate('/inventory'), 2000);
    }
  }, [user, navigate, userRol]);
  ...
  const loadItem = async (itemId: string) => {
    try {
      ...
      setError(`Error al cargar el item: ${errorMsg}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let response;
      if (isEdit && id) {
        response = await inventoryApi.updateItem(id, payload);
        console.log('✅ [CreateItemPage] Item actualizado:', response);
      } else {
        response = await inventoryApi.createItem(payload);
        console.log('✅ [CreateItemPage] Item creado:', response);
      }

      navigate('/inventory');
    } catch (err) {
      const errorMsg = getErrorMessage(err);
      console.error('❌ [CreateItemPage] Error:', err);
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ maxWidth: 600, margin: '2rem auto', padding: '0 1rem' }}>
      <button
        onClick={() => navigate('/inventory')}
        style={{ marginBottom: '1rem', background: 'none', border: '1px solid var(--color-rune-gray)', color: 'var(--color-rune-gray)', padding: '0.4rem 1rem', cursor: 'pointer', borderRadius: 4 }}
      >
        ← Volver
      </button>

      <h1 style={{ color: 'var(--color-gold, #f0c040)', marginBottom: '1.5rem' }}>
        {isEdit ? '✏️ Editar Carta' : '✨ Crear Nueva Carta'}
      </h1>

      {error && (
        <div style={{ color: '#ff4444', marginBottom: '1rem', padding: '0.75rem', border: '1px solid #ff4444', borderRadius: 4 }}>
          ⚠️ {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.4rem' }}>
            {form.habilidades?.map((h, i) => (
              <span key={i} style={tagStyle}>
                {h} <button type="button" onClick={() => removeTag('habilidades', i)} style={tagRemoveStyle}>×</button>
              </span>
            ))}
          </div>
        </label>
        ...
        <label>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.4rem' }}>
            {form.efectos?.map((ef, i) => (
              <span key={i} style={tagStyle}>
                {ef} <button type="button" onClick={() => removeTag('efectos', i)} style={tagRemoveStyle}>×</button>
              </span>
            ))}
          </div>
        </label>
        ...
        >
          {loading ? (isEdit ? 'Actualizando...' : 'Creando...') : (isEdit ? '✏️ Actualizar Carta' : '✨ Crear Carta')}
        </button>
      </form>
    </div>
  );
...
const tagStyle: React.CSSProperties = {
  ...
  padding: '0.2rem 0.6rem', fontSize: '0.85rem', color: '#ccc',
