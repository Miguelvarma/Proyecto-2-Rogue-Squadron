  ...
  adminUpdate = async (
    ...
    try {
      if (req.user!.rol !== 'ADMIN') {
        res.status(403).json({ error: 'Solo administradores pueden actualizar items' });
        return;
      }
      ...
      logInventoryEvent('item.adminUpdated', {
        itemId: id,
        userId: req.user!.userId,
      });

      res.json({
        message: 'Ítem actualizado exitosamente por admin',
        data: result
      });
    } catch (error) {
      next(error);
